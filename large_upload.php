...
define('FILE_PATH_PHY', dirname(__FILE__) . DIRECTORY_SEPARATOR);
...
if (empty($tmp_file)) {
	...
	$tmp_file_dir = FILE_PATH_PHY . date('Ymd') . DIRECTORY_SEPARATOR;
	if (!is_dir($tmp_file_dir)) {
		mkdir($tmp_file_dir, '777', true);
	}
	$tmp_file = $tmp_file_dir . createRandFileName() . '.tmp';
...
if ($file_size == $send) {		//文件大小相等，所以已经上传完毕
	$tmp_file_name = substr($tmp_file, strrpos($tmp_file, DIRECTORY_SEPARATOR));
	$tmp_file_name = substr($complete_file_name, 0, strrpos($complete_file_name, '.'));
	
	rename($tmp_file, $tmp_file_dir . $tmp_file_name . $file_ext);
